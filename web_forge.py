...
st.markdown("Interface de Manufatura de Segurança (Design-Time) para criação autônoma de conectores.")
...
with col1:
    ...
    vendor = st.text_input("Fornecedor / Sistema", value="Cisco ASA", help="Ex: Fortinet, AWS CloudTrail, etc.")
    artifact_id_input = st.text_input("ID do Artefato (Fingerprint)", value="cisco_asa", help="Um identificador curto e limpo.")
    ...
    if st.button("🧠 Analisar Formato com IA (Claude)", use_container_width=True):
        if not forge_ai.available():
            st.error("Erro: A IA não está disponível. Defina a variável ANTHROPIC_API_KEY no seu ambiente.")
...
with col2:
    ...
    if st.session_state["derived_profile"]:
        st.markdown("Valide o perfil semântico gerado pelo LLM. Você pode editar os parâmetros críticos antes do deploy.")
        
        prof = st.session_state["derived_profile"]
        
        # Toggles Visuais
        st.write("**Engine de Parse:**")
        engine = st.radio("Selecione a engine", ["regex", "keyvalue"], index=0 if prof["parse"]["engine"] == "regex" else 1, horizontal=True)
        prof["parse"]["engine"] = engine
        
        if engine == "regex":
            pattern = st.text_input("Regex (Grupos Nomeados)", value=prof["parse"].get("pattern", ""))
        ...
        if prof.get("reasoning") and len(prof["reasoning"]) > 0:
            ...
            risk = col_b.selectbox("Risco", ["Low", "Medium", "High", "Critical"], index=["Low", "Medium", "High", "Critical"].index(first_rule["set"]["risk"]))
...
with col3:
    ...
    if st.session_state["derived_profile"]:
        ...
        st.markdown("---")
        if st.button("🚀 Compilar & Assinar Deploy (Registry)", type="primary", use_container_width=True):
