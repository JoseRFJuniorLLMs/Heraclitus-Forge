...
with st.sidebar:
    ...
    menu_navegacao = st.radio(
        ...
        [
            "Central de Comando (SOC)",
            "Linha do Tempo Forense",
            "Grafo de Relações",
            "Geometria de Produto",
            "Painel Executivo / Governança",
        ],
        index=0,
    )
    st.markdown("---")
...
if menu_navegacao == "Central de Comando (SOC)":
    ...
    with c1:
        st.markdown(
            f'<div class="forensic-card"><div class="forensic-title">Total de Eventos Selados</div><div class="forensic-value">{formatar_sufixo_br(total_events)} <small style="font-size:12px; opacity:0.6;">({total_events:,})</small></div></div>'.replace(
                ",", "."
            ),
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            '<div class="forensic-card"><div class="forensic-title">Volume em Disco</div><div class="forensic-value">30.14 GB</div></div>',
            unsafe_allow_html=True,
        )
    with c3:
        st.markdown(
            '<div class="forensic-card"><div class="forensic-title">Eventos Suspeitos</div><div class="forensic-value">2.365</div></div>',
            unsafe_allow_html=True,
        )
    with c4:
        st.markdown(
            '<div class="forensic-card"><div class="forensic-title">Verificação de Log (DEMO)</div><div class="forensic-value">117/118 OK</div></div>',
            unsafe_allow_html=True,
        )
    ...
    for _idx, row in df_log_pipeline.iterrows():
        data_bloco = row["Data"]
...
# ==============================================================================
    ...
    with col_t1:
        st.metric(
            "LSN Reconstruído no Alvo",
            f"{formatar_sufixo_br(estado_ultimo['LSN_Acumulado'])} ({estado_ultimo['LSN_Acumulado']:,})".replace(
                ",", "."
            ),
        )
    with col_t2:
...
# ==============================================================================
    ...
    st.markdown(
        "Mova a linha do tempo para ver o surgimento das conexões. Os nós agora usam ícones limpos e o texto fica sempre visível."
    )
    ...
    svg_build = []
    svg_build.append(
        '<div class="forensic-card" style="width:100%; box-sizing:border-box;"><svg viewBox="0 0 880 280" style="width:100%; height:auto;">'
    )
    ...
    for edge in edges_config:
        if lsn_atual >= edge["lsn_start"]:
            ...
            svg_build.append(
                f'<line x1="{n_start["x"]}" y1="{n_start["y"]}" x2="{n_end["x"]}" y2="{n_end["y"]}" stroke="#f85149" stroke-width="2.5" opacity="0.8" />'
            )
            svg_build.append(
                f'<text x="{(n_start["x"] + n_end["x"]) / 2}" y="{(n_start["y"] + n_end["y"]) / 2 - 8}" fill="#d29922" font-size="10" font-weight="bold" text-anchor="middle">{edge["weight"]}</text>'
            )
    ...
    for node in nodes_config:
        if lsn_atual >= node["lsn_start"]:
            ...
            svg_build.append(
                f'<text x="{node["x"]}" y="{node["y"] + 6}" font-size="24" text-anchor="middle">{node["icon"]}</text>'
            )

            # Adiciona um círculo luminoso sutil de alerta nos nós recém-criados
            if lsn_atual - node["lsn_start"] < 15_000_000:
                svg_build.append(
                    f'<circle cx="{node["x"]}" cy="{node["y"]}" r="16" fill="none" stroke="#f85149" stroke-width="2" opacity="0.7"/>'
                )

            # Desenha o texto do rótulo travado logo abaixo do ícone (sempre visível em var(--text-color))
            svg_build.append(
                f'<text x="{node["x"]}" y="{node["y"] + 26}" fill="var(--text-color)" font-size="11" font-weight="700" text-anchor="middle" style="fill: var(--text-color) !important;">{node["label"]}</text>'
            )
    ...
    st.markdown(
        f"**Análise Forense no LSN:** `{lsn_atual:,}` | Nós Ativos: `{sum(1 for n in nodes_config if lsn_atual >= n['lsn_start'])}/8`".replace(
            ",", "."
        )
    )
...
# ==============================================================================
    ...
    geo_stats = {
        "Espaço Hiperbólico (H³²)": {
            "vol": int(total_events * 0.55),
            "desc": "Árvores de linhagem causal, indexação HNSW hiperbólica e traces do operador WHY().",
            "cor": "#1f6feb",
        },
        "Espaço Esférico (S⁸)": {
            "vol": int(total_events * 0.25),
            "desc": "Mapeamento de relações cíclicas recorrentes e detecção de anéis de fraude.",
            "cor": "#2ea44f",
        },
        "Espaço Euclidiano (E⁸)": {
            "vol": int(total_events * 0.20),
            "desc": "Atributos planos lineares tradicionais e índices de texto invertido.",
            "cor": "#a371f7",
        },
    }
    ...
    df_chart_geo = pd.DataFrame(
        [{"Espaço Geométrico": k, "Registros Alocados": v["vol"]} for k, v in geo_stats.items()]
    )
    st.bar_chart(
        df_chart_geo, x="Espaço Geométrico", y="Registros Alocados", use_container_width=True
    )
...
# ==============================================================================
    ...
    controles = [
        {
            "Controle": "Imutabilidade Completa do Log",
            "Norma": "PPSI / ISO 27001 A.12.4",
            "Status": "CONFORME",
        },
        {
            "Controle": "Carimbo de Tempo Legal Sincronizado",
            "Norma": "MP 2.200-2 / SERPRO ICP-Brasil",
            "Status": "CONFORME",
        },
    ]
...
st.markdown("---")
st.caption(
    "HeraclitusDB Telemetry Layer v0.1.0 · Grafos baseados em ícones vetoriais e responsividade 100% elástica corrigida."
)
